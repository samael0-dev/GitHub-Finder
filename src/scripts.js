    ...
    try {
        ...
        if(index<=8) {
            if((index)%2==0) {
                ...
                await repositorycard.insertAdjacentHTML("afterend",`<div class="h-[150px] w-[430px] max-md:w-[370px] mb-3 col-span-1 rounded-3xl shadow-[-1px_3px_13px_10px_rgba(0,_0,_0,_0.1)] bg-gray-100"><div class="flex flex-col"><span class="ml-4 mt-3 text-blue-500 text-[20px] font-bold">${await element.name}</span><div class="ml-4">→${await description}</div><div class="ml-4 mt-2 font-bold">🟢${await language(element)}</div></div></div>`);
